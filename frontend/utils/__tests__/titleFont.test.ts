...
test('wide screens keep the full 52px title', () => {
  assert.equal(titleFontSize(460), 52);
...
test('narrow screens shrink the title below 52', () => {
  ...
  assert.ok(narrow >= 37, `expected >= 37, got ${narrow}`);
});
...
test('longest name fits within available width at every size', () => {
  ...
  // Available width: window width minus 104 total chrome (container 56 + heroPanel 40 + 8 safety margin).
  for (let w = 320; w <= 500; w += 10) {
    const f = titleFontSize(w);
    const rendered = 9 * (0.7 * f + 2);
    assert.ok(rendered <= w - 104, `at width ${w}, fontSize ${f} renders ${rendered}px > ${w - 104}px`);
  }
});
